...
function RoutineEditor({ routineId, onClose }: { routineId: string | null; onClose: () => void }) {
  ...
  const filtered = useMemo(() => exercises.filter((exercise) => exercise.name.toLowerCase().includes(query.toLowerCase())), [exercises, query]);
...
const styles = StyleSheet.create({
  ...
  configField: { flex: 1 },
  configLabel: { fontSize: 10, marginBottom: 4 },
  reorder: { flexDirection: 'row', gap: 18, marginTop: 2 },
  reorderText: { fontSize: 12, fontWeight: '700' },
  catalogRow: { borderRadius: 15, borderWidth: 1, padding: 14, marginBottom: 8, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
